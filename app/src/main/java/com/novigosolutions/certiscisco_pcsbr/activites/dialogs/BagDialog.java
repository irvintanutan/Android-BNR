...
public class BagDialog extends Dialog implements View.OnClickListener, IOnScannerData, ApiCallback, RecyclerViewClickListener {
    ...
    public BagDialog(Context context, int TransportMasterId, DialogResult mDialogResult) {
...
//                        String barcode2 = txtbag2.getText().toString().replaceAll("[^a-zA-Z0-9]", "").toUpperCase();
                    ...
                    if (bar_code_list.size() > 0) {
                        for (int i = 0; i < bar_code_list.size(); i++) {
                            ...
                            Bags bag = new Bags();
                            bag.TransportMasterId = TransportMasterId;
                            bag.firstbarcode = bagcodes.length > 0 ? bagcodes[0] : "";
                            bag.secondbarcode = bagcodes.length > 1 ? bagcodes[1] : "";
                            bag.save();
                        }
                        ...
                        Toast.makeText(context, "Add bag", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
